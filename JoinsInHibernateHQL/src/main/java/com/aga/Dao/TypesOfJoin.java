	...
	public static void main(String[] args) {
		...
		try {
			 ...
			 */
				...
				/*
				 * 3.	RIGHT OUTER JOIN:
				 						It has the same functionality as the LEFT OUTER JOIN.
				 						But the only diff is that the table on the right is the source and all of its rows
				 						[that do not match a row in the joined table] are included in the result.
				 	
				 	e.g.: FROM Authores a RIGHT OUTER JOIN a.bookeses
				 */
				
				Query rightQuery= session.createQuery("FROM Books b RIGHT OUTER JOIN b.authors");
